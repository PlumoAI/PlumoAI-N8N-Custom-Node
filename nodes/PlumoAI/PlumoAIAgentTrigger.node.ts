	...
	methods = {
		loadOptions: {
			...
			async getAgents(this: ILoadOptionsFunctions) {
				...
				filter((project: any) => project.location_fid == this.getNodeParameter('workspace', 0) && project.template_proj_type_fid==8 && !project?.project_aiagent_config).map((project: any) => ({
